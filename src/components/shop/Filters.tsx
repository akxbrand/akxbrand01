  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // After categories are loaded, apply initial filters if any are selected
        if (selectedCategories.length > 0 || selectedSubcategories.length > 0) {
          // Verify selected categories exist in loaded data
          const validCategories = selectedCategories.filter(catId =>
            categoriesWithSubcategories.some(cat => cat.id === catId)
          );
          
          // Verify selected subcategories exist in loaded data
          const validSubcategories = selectedSubcategories.filter(subId =>
            categoriesWithSubcategories.some(cat =>
              cat.subcategories.some(sub => sub.id === subId)
            )
          );

          // Update selected filters if they've changed
          if (validCategories.length !== selectedCategories.length) {
            onCategoryChange(validCategories[0] || '');
          }
          if (validSubcategories.length !== selectedSubcategories.length) {
            onSubcategoryChange(validSubcategories[0] || '');
          }
        }
      } catch (error) {
