...
export async function PUT(req: Request) {
  try {
    await prisma.$connect();
    const data = await req.json();
    const { id, name, description, categoryId, status, imageUrl } = data;

    if (!id || !name || !categoryId || !imageUrl) {
      return NextResponse.json(
        { success: false, error: 'ID, name, category ID, and image URL are required' },
        { status: 400 }
      );
    }
